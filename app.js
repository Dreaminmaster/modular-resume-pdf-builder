const STORAGE_KEY = 'modular_resume_builder_v7';
...
function isMeaningfullyEmpty() {
  ...
  if ([p.name, p.title, p.phone, p.email, p.location, p.website, state.modules.summary.fields.summary].some(f => text(f.value))) return false;
  const listModules = ['education', 'internships', 'projects', 'research', 'awards', 'achievements', 'skills', 'certificates', 'languages', 'links'];
  return !listModules.some(k => state.modules[k].items.some(item => Object.values(item).some(v => v && typeof v === 'object' && 'value' in v && text(v.value)) || visibleBullets(item).length));
}
...
function exportJsonResume() { const p = state.modules.personalInfo.fields; const basics = { name: text(p.name.value), label: text(p.title.value), email: text(p.email.value), phone: text(p.phone.value), url: text(p.website.value), summary: text(state.modules.summary.fields.summary.value), image: text(p.avatar.value), location: { address: text(p.location.value) }, profiles: state.modules.links.items.filter(i => i.visible).map(i => ({ network: text(i.title.value), url: text(i.url.value), username: text(i.desc.value) })) }; const education = state.modules.education.items.filter(i => i.visible).map(i => ({ institution: text(i.school.value), studyType: text(i.degree.value), area: text(i.major.value), startDate: parseDateStart(i.date.value), endDate: parseDateEnd(i.date.value), score: visibleBullets(i).map(b => b.value).join(' | ') })); const work = state.modules.internships.items.filter(i => i.visible).map(i => ({ name: text(i.org.value), position: text(i.role.value), location: text(i.location.value), startDate: parseDateStart(i.date.value), endDate: parseDateEnd(i.date.value), summary: text(i.name.value), highlights: visibleBullets(i).map(b => b.value) })); const projects = state.modules.projects.items.filter(i => i.visible).map(i => ({ name: text(i.name.value), description: [text(i.role.value), text(i.org.value)].filter(Boolean).join(' · '), startDate: parseDateStart(i.date.value), endDate: parseDateEnd(i.date.value), highlights: visibleBullets(i).map(b => b.value), url: '' })); const skills = state.modules.skills.items.filter(i => i.visible).map(i => ({ name: text(i.category.value) || 'Skills', keywords: text(i.content.value).split(/[,，、]/).map(s => s.trim()).filter(Boolean) })); const languages = state.modules.languages.items.filter(i => i.visible).map(i => ({ language: text(i.language.value), fluency: text(i.level.value) })); const awards = state.modules.awards.items.filter(i => i.visible).map(i => ({ title: text(i.title.value), awarder: text(i.issuer.value), date: text(i.date.value), summary: visibleBullets(i).map(b => b.value).join(' | ') })); const certificates = state.modules.certificates.items.filter(i => i.visible).map(i => ({ name: text(i.name.value), issuer: text(i.issuer.value), date: text(i.date.value) })); const publications = state.modules.achievements.items.filter(i => i.visible).map(i => ({ name: text(i.title.value), publisher: text(i.status.value), releaseDate: text(i.date.value), summary: [text(i.type.value), ...visibleBullets(i).map(b => b.value)].filter(Boolean).join(' | ') })); const volunteer = state.modules.research.items.filter(i => i.visible).map(i => ({ organization: text(i.org.value), position: text(i.role.value), startDate: parseDateStart(i.date.value), endDate: parseDateEnd(i.date.value), summary: text(i.name.value), highlights: visibleBullets(i).map(b => b.value) })); const interests = state.modules.skills.items.filter(i => i.visible && !text(i.category.value)).flatMap(i => text(i.content.value).split(/[,，、]/).map(s => s.trim()).filter(Boolean)).map(v => ({ name: v })); return { basics, work, volunteer, education, awards, certificates, publications, skills, languages, projects, interests, references: [], meta: { canonical: 'https://jsonresume.org/schema/' } }; }
function importJsonResume(data) { state = defaultState(); const b = data.basics || {}; const pf = state.modules.personalInfo.fields; pf.name.value = b.name || ''; pf.title.value = b.label || ''; pf.email.value = b.email || ''; pf.phone.value = b.phone || ''; pf.website.value = b.url || ''; pf.location.value = (b.location && (b.location.address || b.location.city || b.location.region || b.location.countryCode)) || ''; pf.avatar.value = b.image || ''; pf.avatar.visible = !!b.image; if (b.summary) state.modules.summary.fields.summary.value = b.summary; if (safeArray(b.profiles).length) state.modules.links.items = b.profiles.map(pr => ({ id: uid('link'), visible: true, title: createField('链接名称', pr.network || '', true), url: createField('链接地址', pr.url || '', true), desc: createField('说明', pr.username || '', true), bullets: [] })); if (safeArray(data.education).length) state.modules.education.items = data.education.map(e => ({ id: uid('edu'), visible: true, school: createField('学校', e.institution || '', true), degree: createField('学历/学位', e.studyType || '', true), major: createField('专业', e.area || '', true), date: createField('时间', [e.startDate, e.endDate].filter(Boolean).join(' - '), true), location: createField('地点', '', true), bullets: e.score ? [createBullet(e.score, true)] : [] })); if (safeArray(data.work).length) state.modules.internships.items = data.work.map(w => ({ id: uid('intern'), visible: true, name: createField('名称', w.summary || '', true), role: createField('岗位/角色', w.position || '', true), org: createField('单位/平台', w.name || '', true), date: createField('时间', [w.startDate, w.endDate].filter(Boolean).join(' - '), true), location: createField('地点', w.location || '', true), bullets: safeArray(w.highlights).map(h => createBullet(h, true)) })); if (safeArray(data.projects).length) state.modules.projects.items = data.projects.map(p => ({ id: uid('project'), visible: true, name: createField('名称', p.name || '', true), role: createField('岗位/角色', p.description || '', true), org: createField('单位/平台', '', true), date: createField('时间', [p.startDate, p.endDate].filter(Boolean).join(' - '), true), location: createField('地点', '', true), bullets: safeArray(p.highlights).map(h => createBullet(h, true)) })); if (safeArray(data.volunteer).length) state.modules.research.items = data.volunteer.map(v => ({ id: uid('research'), visible: true, name: createField('名称', v.summary || '', true), role: createField('岗位/角色', v.position || '', true), org: createField('单位/平台', v.organization || '', true), date: createField('时间', [v.startDate, v.endDate].filter(Boolean).join(' - '), true), location: createField('地点', '', true), bullets: safeArray(v.highlights).map(h => createBullet(h, true)) })); if (safeArray(data.skills).length) state.modules.skills.items = data.skills.map(s => ({ id: uid('skill'), visible: true, category: createField('技能分类', s.name || '', true), content: createField('技能内容', safeArray(s.keywords).join('、'), true), bullets: [] })); if (safeArray(data.languages).length) state.modules.languages.items = data.languages.map(l => ({ id: uid('lang'), visible: true, language: createField('语言', l.language || '', true), level: createField('水平', l.fluency || '', true), bullets: [] })); if (safeArray(data.awards).length) state.modules.awards.items = data.awards.map(a => ({ id: uid('award'), visible: true, title: createField('奖项名称', a.title || '', true), issuer: createField('颁发单位', a.awarder || '', true), date: createField('时间', a.date || '', true), bullets: a.summary ? [createBullet(a.summary, true)] : [] })); if (safeArray(data.certificates).length) state.modules.certificates.items = data.certificates.map(c => ({ id: uid('cert'), visible: true, name: createField('证书名称', c.name || '', true), issuer: createField('颁发机构', c.issuer || '', true), date: createField('时间', c.date || '', true), bullets: [] })); if (safeArray(data.publications).length) state.modules.achievements.items = data.publications.map(pu => ({ id: uid('achievement'), visible: true, title: createField('成果名称', pu.name || '', true), type: createField('类型', 'Publication', true), date: createField('时间', pu.releaseDate || '', true), status: createField('状态', pu.publisher || '', true), bullets: pu.summary ? [createBullet(pu.summary, true)] : [] })); render(); }
...
function getPreviewOrder(template) {
  ...
  if (template === 'engineering') {
    const first = ['projects', 'skills', 'internships', 'achievements', 'education'];
    return [...first.filter(k => manual.includes(k)), ...manual.filter(k => !first.includes(k))];
  }
  if (template === 'academic') {
    const first = ['education', 'research', 'achievements', 'awards', 'languages'];
    return [...first.filter(k => manual.includes(k)), ...manual.filter(k => !first.includes(k))];
  }
  if (template === 'product') {
    const first = ['projects', 'internships', 'skills', 'links', 'education'];
    return [...first.filter(k => manual.includes(k)), ...manual.filter(k => !first.includes(k))];
  }
  if (template === 'en-minimal') {
    const first = ['education', 'internships', 'projects', 'skills', 'languages', 'achievements'];
    manual = [...first.filter(k => manual.includes(k)), ...manual.filter(k => !first.includes(k))];
    const mapped = []; let inserted = false;
    manual.forEach(k => { if (EXPERIENCE_KEYS.includes(k)) { if (!inserted) { mapped.push('experience'); inserted = true; } } else mapped.push(k); });
    return mapped;
  }
  return manual;
}
...
function renderHeader() {
  ...
  let extra = [];
  if (state.settings.template !== 'en-minimal') extra = ['gender', 'birth', 'political'].map(k => f[k]).filter(isFieldVisible).map(x => `<span class="contact-item">${escapeHtml(x.value)}</span>`);
...
function renderEmptyState() {
  const msg = state.settings.languageMode === 'zh' ? '请先填写信息池，勾选后的内容会出现在这里。' : 'Start by filling your information pool.';
  return `<div class="preview-meta-card"><strong>${escapeHtml(msg)}</strong></div>`;
}
...
function renderPreview() {
  if (isMeaningfullyEmpty()) {
    resumePage.innerHTML = renderEmptyState();
    return requestAnimationFrame(checkPageOverflow);
  }
  const template = state.settings.template; const order = getPreviewOrder(template);
  if (template === 'campus') { resumePage.innerHTML = renderCampusLayout(order); return requestAnimationFrame(checkPageOverflow); }
  const parts = [renderPreviewMeta(), renderHeader()]; order.forEach(key => { if (key === 'experience') parts.push(renderCombinedExperience()); else if (key === 'summary') parts.push(renderSummary()); else parts.push(renderListModule(key)); }); parts.push(renderPageFooter()); resumePage.innerHTML = parts.join(''); requestAnimationFrame(checkPageOverflow);
}
