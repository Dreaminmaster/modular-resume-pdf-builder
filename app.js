...
function renderPageFooter() { return `<div class="page-footer export-exclude"><span>${TEMPLATE_META[state.settings.template].name}</span><span>Generated by Modular Resume PDF Builder</span></div>`; }
function renderPreviewMeta() { const modulesVisible = moduleDefs().filter(def => state.modules[def.key].visible).length; return `<div class="preview-meta-card export-exclude no-print">当前模板强调：${escapeHtml(TEMPLATE_META[state.settings.template].hint)} 当前已启用模块 ${modulesVisible} 个。</div>`; }
